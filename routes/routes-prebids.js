...
router.post('/api/pre-bids', authenticate, async (req, res) => {
	try {
		...
		if (auction.status !== 'upcoming') {
			return res
				.status(400)
				.json({ message: 'Pre-bids can only be queued on upcoming auctions' });
		}
		...
		if (getUserPreBid.get(auction_id, user_id)) {
			return res
				.status(409)
				.json({ message: 'You already have a pre-bid queued for this auction' });
		}
		...
		res
			.status(200)
			.json({ message: 'Pre-bid queued', pre_bid_id: lastInsertRowid });
	} catch (err) {
...
router.put(
	'/api/pre-bids/:pre_bid_id/cancel',
	authenticate,
	async (req, res) => {
		try {
			...
			if (!preBid)
				return res.status(400).json({ message: 'Pre-bid already cancelled' });
			if (preBid.user_id !== req.userId)
				return res
					.status(403)
					.json({ message: 'You can only cancel your own pre-bid' });
